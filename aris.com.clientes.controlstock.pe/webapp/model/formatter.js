sap.ui.define([
	...
	return {
		...
		absImgPath: function (sCodigo) {
			if (!sCodigo) return "";
			const oModel = sap.ui.getCore().getModel("oModelImages");
			if (!oModel) return "";
			const clean = String(sCodigo).trim().toUpperCase();
			const map = oModel.getProperty("/MapByCode") || {};
			return map[clean] || "";
		},


		getRowClass: function (sValue) {
			// si es null/undefined, devuelve vacío
			if (sValue === null || sValue === undefined) {
				return "";
			}
			// convierto string a número si hace falta
			var iVal = parseInt(sValue, 10);

			// devuelvo la clase que definí en CSS
			return (iVal === 0) ? "myRedText" : "";
		},
		stockDisplay: function (total, detalle) {
		...
		pickGroupOrDetail: function (total, detalle) {
			if (total !== undefined && total !== null && total !== "") {
				return total;
			}
			return detalle || "";
		},
		fmtShowPallets: function (v, sTipo) {
            sTipo = String(sTipo || "TODOS").toUpperCase();
            return (sTipo === "PALLETS" || sTipo === "TODOS") ? (v ?? "") : "";
        },

        fmtShowCajas: function (v, sTipo) {
            sTipo = String(sTipo || "TODOS").toUpperCase();
            return (sTipo === "CAJAS" || sTipo === "TODOS") ? (v ?? "") : "";
        },
