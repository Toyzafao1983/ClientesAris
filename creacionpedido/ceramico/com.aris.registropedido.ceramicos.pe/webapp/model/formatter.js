sap.ui.define([
	...
	return {
		...
		formatMonto2Dec: function (vValue) {
			...
			// Si viene con coma decimal "1234,56" lo normalizamos
		...
		pickGroupOrDetail: function (isGroup, totalVal, detailVal) {
			...
			// Normaliza nulos/vacío
			if (v == null || v === "") {
				return "";
			}

			// Si por algún motivo viene boolean, conviértelo a número
			if (typeof v === "boolean") {
				v = v ? 1 : 0;
			}

			// Usa siempre el formateador de 2 decimales
			return this.formatMonto2Dec(v);
		},
		...
		stockDisplay: function (isGroup, totalStockM2, stockM2) {
			...
			// Usa siempre el formateador de 2 decimales
			return this.formatMonto2Dec(val);
		},
		formatCreditUSD: function (vValue) {
			...
			return sFormatted + " PEN";
		},
		formatMaterialWithDesc: function (sMat, sDesc) {
			if (!sMat && !sDesc) {
				return "";
			}
			if (!sDesc) {
		...
		_bpEq: function (bp1, bp2) {
			const a = (bp1 || "").toString().trim();
			const b = (bp2 || "").toString().trim();
			return !!a && !!b && a === b;
		},
