...
sap.ui.define([
    ...
    "com/aris/registropedido/textiles/pe/util/utilUI",
    ...
    formatter: Formatter;
    return BaseController.extend("com.aris.registropedido.textiles.pe.controller.Main", {
        onInit() {
            that = this;
        ...
        handleRouteMatched: function (bInit) {
            ...
            that._getCreditDispo(),that._getDatClient(),that._getClientPet(),that._getBPVendedor()
                    ...
                    try {
                        ...
                        that.oModelData.setProperty("/oFilterMaterial", values[2].oResults);
                        that.oModelProyect.setProperty("/ListaBrands", values[2].aBrands);
                        that.oModelProyect.setProperty("/ListaMaterialGroups", values[2].aMaterialGroups);
                        ...
                        that.getModel("oModelUser").setProperty(
                            "/sNameComp",
                            oIASUser.name.givenName + " " + oIASUser.name.familyName
                        );
        ...
        onClearFilters: function () {
         ...
         _onInitSellerDefault: function () {
            ...
            const bIsInterno  = !!oUser.getProperty("/bIsInterno");
            const bIsVendedor = !!oUser.getProperty("/bIsVendedor");

            if (!bIsInterno || !bIsVendedor) {
                // Cliente o Coordinador → no se auto rellena el MultiInput
                return;
            }
            const sBP   = oUser.getProperty("/bBP") || oUser.getProperty("/bBPFinal");
       ...
       _onPressExecute: function () {
                ...
                try {
                    ...
                    return new Promise(function (resolve, reject) {
                        ...
                        var sPath = jQuery.sap.getModulePath("com.aris.registropedido.textiles.pe") +
                            "/S4HANA/sap/opu/odata/sap/ZSDB_PORTALCLIENTES/Customer?$filter=SalesOrganization eq '1110' and DistributionChannel eq 'C1' and DistributionChannel eq 'C2' and Division eq 'S1'&$format=json&sap-language=es-ES";
                            Services.getoDataERPSync(that, sPath, function (result) {
                                ...
                                error: function (message) {
                                    ...
                                    var oTable = that._byId("TableClient").getItems().length > 0
                                        ? that._byId("TableClient").getItems()[0]
                                        : null;
                    ...
                    .then((values) => {
                        ...
                        if (!that._dialogFilterSeller) {
                            that._dialogFilterSeller = sap.ui.xmlfragment(
                                ...
                                "com.aris.registropedido.textiles.pe.view.dialogs.FilterSeller",
                                that
                            );
                    ...
                    .then((values) => {
                        ...
                        if (!that._dialogFilterCodClient) {
                            that._dialogFilterCodClient = sap.ui.xmlfragment(
                                ...
                                "com.aris.registropedido.textiles.pe.view.dialogs.FilterCodClient",
                                that
                            );
                    ...
                    .then((values) => {
                        ...
                        if (!that._dialogFilterRazonSocial) {
                            that._dialogFilterRazonSocial = sap.ui.xmlfragment(
                                ...
                                "com.aris.registropedido.textiles.pe.view.dialogs.FilterRazonSocial",
                                that
                            );
                    ...
                    .then((values) => {
                        ...
                        if (!that._dialogFilterDocument) {
                            that._dialogFilterDocument = sap.ui.xmlfragment(
                                ...
                                "com.aris.registropedido.textiles.pe.view.dialogs.FilterDocument",
                                that
                            );
        ...
        // Seller (puede estar en Seller o txt13)
        if (Array.isArray(jFilter.fSeller) && jFilter.fSeller.length > 0) {
            const aSeller = jFilter.fSeller
                .filter(k => k && k.trim() !== "")
                .map(k => new F({
                    filters: [
            ...
            aFilters.push(new F({ filters: aSeller, and: false }));
        }

        // Código Cliente (Customer)
        if (Array.isArray(jFilter.fCodClient) && jFilter.fCodClient.length > 0) {
            const aCod = jFilter.fCodClient
                .filter(k => k && k.trim() !== "")
                .map(k => new F("Customer", FO.EQ, k));
            aFilters.push(new F({ filters: aCod, and: false }));
        }

        // Razón Social (solo CustomerFullName)
        if (Array.isArray(jFilter.fRazSocial) && jFilter.fRazSocial.length > 0) {
            const aRaz = jFilter.fRazSocial
                .filter(k => k && k.trim() !== "")
                .map(k => new F("CustomerFullName", FO.Contains, k));
            aFilters.push(new F({ filters: aRaz, and: false }));
        }

        // Razón Social extendida (CustomerFullName + TaxNumbers)
        if (Array.isArray(jFilter.fRazSocial) && jFilter.fRazSocial.length > 0) {
            const aRazEx = jFilter.fRazSocial
                .filter(k => k && k.trim() !== "")
                .map(k => new F({
                    filters: [
                        new F("CustomerFullName", FO.Contains, k),
            ...
            aFilters.push(new F({ filters: aRazEx, and: false }));
        }

        // Documento (solo por TaxNumbers + CustomerFullName)
        if (Array.isArray(jFilter.fDocument) && jFilter.fDocument.length > 0) {
            const aDoc = jFilter.fDocument
                .filter(k => k && k.trim() !== "")
                .map(k => new F({
                    filters: [
                        new F("TaxNumber1", FO.EQ, k),
                        new F("TaxNumber2", FO.EQ, k),
                        new F("TaxNumber3", FO.EQ, k),
                        new F("TaxNumber4", FO.EQ, k),
                        new F("TaxNumber5", FO.EQ, k),
                        new F("TaxNumber6", FO.EQ, k),
                        new F("CustomerFullName", FO.Contains, k)
                    ],
                    and: false
                }));
            aFilters.push(new F({ filters: aDoc, and: false }));
        }
