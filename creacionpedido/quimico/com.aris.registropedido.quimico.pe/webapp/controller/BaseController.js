sap.ui.define([
	...
	'sap/m/Token',
	"com/aris/registropedido/quimico/pe/services/ServiceOdata",
	...
	return Controller.extend("com.aris.registropedido.quimico.pe.controller.BaseController", {
		...
		_getPrueba: function () {
			...
			try {
				...
				return new Promise(function (resolve, reject) {
					let sUrl = "";
					if (that.local) {
						const sPath = '/sap/opu/odata/sap/ZODATA_PM_SOLICIT_MAQ_Z4_SRV/EtResponsableSet?sap-language=ES';
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(sPath);
					} else {
						const sPath = jQuery.sap.getModulePath(that.route) + '/S4HANA/sap/opu/odata/sap/ZODATA_PM_SOLICIT_MAQ_Z4_SRV/EtResponsableSet?sap-language=ES';
						sUrl = sPath;
					}
		...
		_getTipMaterialData: function () {
			try {
				...
				return new Promise(function (resolve, reject) {
					let sUrl = "";
					if (that.local) {
						const sPath = "/sap/opu/odata/sap/ZSDB_PORTALCLIENTES/MatGroup" +
							"?$filter=SalesOrganization eq '1120'&$top=10000&$format=json&sap-language=ES";
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(sPath);
					} else {
						const sPath = jQuery.sap.getModulePath(that.route) +
							"/S4HANA/sap/opu/odata/sap/ZSDB_PORTALCLIENTES/MatGroup" +
							"?$filter=SalesOrganization eq '1120'&$top=10000&$format=json&sap-language=ES";
						sUrl = sPath;
					}
		...
		_getReason: function () {
			try {
				...
				return new Promise(function (resolve, reject) {
					let sUrl = "";
					if (that.local) {
						const sPath = "/sap/opu/odata/sap/ZSDB_PORTALCLIENTES/Reason?$filter=SalesDocumentClass eq 'ZGNA'&$top=10000&$format=json&sap-language=ES";
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(sPath);
					} else {
			...
			try {
				...
				return new Promise(function (resolve, reject) {
					let sUrl = "",
						sNumPedido = "";
					if (that.local) {
						const sPath = '/sap/opu/odata/sap/ZODATA_PM_SOLICIT_MAQ_Z4_SRV/EtResponsableSet?sap-language=ES' + sNumPedido;
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(sPath);
					} else {
						const sPath = jQuery.sap.getModulePath(that.route) + '/S4HANA/sap/opu/odata/sap/ZODATA_PM_SOLICIT_MAQ_Z4_SRV/EtResponsableSet?sap-language=ES' + sNumPedido;
						sUrl = sPath;
					}
					if (that.localModel) { resolve(models.JsonMoneda(context)); }
					else {
						Services.getoDataERPSync(that, sUrl, function (result) {
							util.response.validateAjaxGetERPNotMessage(result, {
								success: function (oData, message) {
		...
		_getDatClient: function (sCustomer) {
			try {
				...
				return new Promise(function (resolve, reject) {
					let sUrl = "";
					if (that.local) {
						const sPath = "/sap/opu/odata/sap/ZSDB_PORTALCLIENTES/DataCustomer?$filter=SalesOrganization eq '1120' and DistributionChannel eq 'C1' and Division eq 'S1' and (CustomerDni ne '' or CustomerRuc ne '')&$top=10000&$format=json&sap-language=ES";
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(sPath);
					} else {
						const sPath = jQuery.sap.getModulePath(that.route) + "/S4HANA/sap/opu/odata/sap/ZSDB_PORTALCLIENTES/DataCustomer?$filter=SalesOrganization eq '1120' and DistributionChannel eq 'C1' and Division eq 'S1' and (CustomerDni ne '' or CustomerRuc ne '')&$top=10000&$format=json&sap-language=ES";
						sUrl = sPath;
					}
					Services.getoDataERPSync(that, sUrl, function (result) {
						util.response.validateAjaxGetERPNotMessage(result, {
		...
		_getAddressData: function (sCustomer) {
			try {
				...
				return new Promise(function (resolve, reject) {
					let sUrl = "";
					if (that.local) {
						const sPath = `/sap/opu/odata/sap/ZSDWS_PORTAL_CLIENTES_SRV/iDireccionesSet(Businesspartner='${sCustomer}')?$format=json&sap-language=ES`;
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(sPath);
					} else {
						const sPath = jQuery.sap.getModulePath(that.route) +
							`/S4HANA/sap/opu/odata/sap/ZSDWS_PORTAL_CLIENTES_SRV/iDireccionesSet(Businesspartner='${sCustomer}')?$format=json&sap-language=ES`;
						sUrl = sPath;
					}
					Services.getoDataERPSync(that, sUrl, function (result) {
						util.response.validateAjaxGetERPNotMessage(result, {
		...
		_getTipDocumentData: function () {
			try {
				...
				return new Promise(function (resolve, reject) {
					let sUrl = "";
					if (that.local) {
						const sPath = "/sap/opu/odata/sap/ZSDB_PORTALCLIENTES/PermittedDocuments?$filter=auart eq 'ZCNA' or auart eq 'ZGNA' or auart eq 'ZPES' or auart eq 'ZACN'&$top=10000&$format=json&sap-language=ES";
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(sPath);
					} else {
						const sPath = jQuery.sap.getModulePath(that.route) + "/S4HANA/sap/opu/odata/sap/ZSDB_PORTALCLIENTES/PermittedDocuments?$filter=auart eq 'ZCNA' or auart eq 'ZGNA' or auart eq 'ZPES' or auart eq 'ZACN'&$top=10000&$format=json&sap-language=ES";
						sUrl = sPath;
					}
					Services.getoDataERPSync(that, sUrl, function (result) {
						util.response.validateAjaxGetERPNotMessage(result, {
		...
		_getCreditDispo: function (sPartner, tSalesOrg) {
			try {
				var oResp = {
					"sEstado": "E",
					"oResults": []
				...
				return new Promise(function (resolve, reject) {
					let sUrl = "";
					if (that.local) {
						const sPath = `/sap/opu/odata/sap/ZSDWS_PORTAL_CLIENTES_SRV/EcreditosSet?$filter=Partner eq '${sPartner}' and Segment eq '${tSalesOrg}'&$top=10000&$format=json&sap-language=ES`;
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(sPath);
					} else {
						const sPath = jQuery.sap.getModulePath(that.route) +
							`/S4HANA/sap/opu/odata/sap/ZSDWS_PORTAL_CLIENTES_SRV/EcreditosSet?$filter=Partner eq '${sPartner}' and Segment eq '${tSalesOrg}'&$top=10000&$format=json&sap-language=ES`;
						sUrl = sPath;
					}
					Services.getoDataERPSync(that, sUrl, function (result) {
						util.response.validateAjaxGetERPNotMessage(result, {
		...
		_getMaterialStock: function () {
			try {
				...
				return new Promise((resolve, reject) => {
					let sUrl = "";
					const sBase = "/sap/opu/odata/sap/ZSDB_PORTALCLIENTES/MaterialsConsultation";
					const sParams = "?$filter=SalesOrganization eq '1120' and DistributionChannel eq 'C1'&$top=10000&$format=json&sap-language=ES";

					if (that.local) {
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(`${sBase}${sParams}`);
					} else {
						const sPath = jQuery.sap.getModulePath(that.route) + "/S4HANA" + sBase;
						sUrl = `${sPath}${sParams}`;
					}
					...
					Services.getoDataERPSync(that, sUrl, function (result) {
						util.response.validateAjaxGetERPNotMessage(result, {
							success: function (oData) {
								...
								// ✅ Para sugerencias: solo Material + Description
								const aForSuggestions = aResults.filter(item =>
									item.Material &&
									item.Material.trim() !== "" &&
									item.Description &&
								...
								// ✅ Para listas de marca
								const aBrands = [...new Set(
									aResults
										.map(i => i.Brand)
										.filter(v => v && v.trim() !== "")
								)].sort();

								// ✅ Para listas de grupo de material
								const aMaterialGroups = [...new Set(
									aResults
										.map(i => i.MaterialGroup)
										.filter(v => v && v.trim() !== "")
								)].sort();

								// ✅ Lista de descripciones (opcionalmente desde aForSuggestions)
								const aDescriptions = [...new Set(
									aForSuggestions
										.map(i => i.Description)
										.filter(v => v && v.trim() !== "")
								)].sort();
								...
								// Armamos respuesta
								oResp.sEstado = "S";
								oResp.oResults = aForSuggestions;
								oResp.aBrands = aBrands;
								oResp.aMaterialGroups = aMaterialGroups;
								oResp.aDescriptions = aDescriptions;

								// Cargar en modelos
								const oModelData = that.getView().getModel("oModelData");
								if (oModelData) {
									oModelData.setProperty("/oFilterMaterial", aForSuggestions);
									oModelData.setProperty("/ListDescription", aDescriptions);
									oModelData.setProperty("/ListDescriptionSug", []);
									oModelData.setProperty("/ListaMaterialGroups", aMaterialGroups);
									oModelData.setProperty("/ListBrandCodes", aBrands);
								}

								const oModelProj = that.getView().getModel("oModelProyect");
								if (oModelProj) {
									oModelProj.setProperty("/ListaBrands", aBrands);
									oModelProj.setProperty("/ListaMaterialGroups", aMaterialGroups);
								}
					...
					Services.getoDataERPSync(that, sUrl, function (result) {
						util.response.validateAjaxGetERPNotMessage(result, {
							success: function (oData, message) {
								...
								const oModelData = that.getView().getModel("oModelData");
								if (oModelData) {
									oModelData.setProperty("/ListBrand", aUnique);
									oModelData.setProperty("/ListBrandSug", aUnique);
								}
								oResp.sEstado = "S";
		...
		_getStockDisp: function () {
			try {
				...
				return new Promise(function (resolve, reject) {
					let sUrl = "";
					if (that.local) {
						const sPath = "/sap/opu/odata/sap/ZSDWS_PORTAL_CLIENTES_SRV/I_StockDisponibleSet?$filter=Materialnumber eq '300059' and Salesorganization eq '1120' and Plant eq '1000' and Pedven eq true &$expand=toStockQuimico&$top=10000&$format=json&sap-language=ES";
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(sPath);
					} else {
						const sPath = jQuery.sap.getModulePath(that.route) + "/S4HANA/sap/opu/odata/sap/ZSDWS_PORTAL_CLIENTES_SRV/I_StockDisponibleSet?$filter=Materialnumber eq '300059' and Salesorganization eq '1120' and Plant eq '1000' and Pedven eq true &$expand=toStockQuimico&$top=10000&$format=json&sap-language=ES";
						sUrl = sPath;
					}
					Services.getoDataERPSync(that, sUrl, function (result) {
						util.response.validateAjaxGetERPNotMessage(result, {
		...
		_getAddresTravel: function (sCustomer) {
			try {
				...
				return new Promise((resolve) => {
					...
					console.log("📌 URL FullAddressSet:", sUrl);
					console.log("📌 bIsVendedor:", bIsVendedor);
					console.log("📌 SalesPartner enviado:", sBPUsuario);
					...
					Services.getoDataERPSync(that, sUrl, function (result) {
						util.response.validateAjaxGetERPNotMessage(result, {
							success: function (oData) {
								...
								console.log("RAW FullAddressSet[0]:", aResults && aResults[0]);
		...
		_getPesoProducto: function (sMaterial, sMeins, sUmv, vQuantity) {
			try {
				var oResp = {
					sEstado: "E",
					oResults: []
				...
				return new Promise(function (resolve, reject) {
					...
					const fQty = parseFloat(vQuantity) || 0;
					const sQty = fQty.toFixed(3);

					const sBase = "/sap/opu/odata/sap/ZSDWS_PORTAL_CLIENTES_SRV/MaterialPesoSet";
					const sFilter = `?$filter=Material eq '${sMaterial}' and Meins eq '${sMeins}' and Umv eq '${sUmv}' and Quantity eq ${sQty}&$top=10000&$format=json&sap-language=ES`;

					if (that.local) {
						const sPath = sBase + sFilter;
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(sPath);
					} else {
						const sPath = jQuery.sap.getModulePath(that.route) + "/S4HANA" + sBase + sFilter;
						sUrl = sPath;
					}
		...
		_getSalesOrgByBP: function (sBP) {
			...
			return new Promise((resolve) => {
				...
				const sFilter = `$filter=BP eq '${sBP}'&$expand=toBpSalesOrganization&$top=10000&$format=json&sap-language=ES`;
				let sUrl = "";
		...
		_getAnticipo: function (sCustomer, sCurrency) {
			...
			try {
				...
				return new Promise(function (resolve) {
					...
					const sFilter = "$filter=Customer eq '" + sCustomer + "' and Currency eq '" + sCurrency + "'and SalesOrganization eq '1120'";
		...
		_setLanguageModel: function (langKey) {
			var bundleName;
			if (langKey === "esp") {
				bundleName = "com.aris.registropedido.quimico.pe.i18n.i18n_esp";
			} else if (langKey === "ing") {
				bundleName = "com.aris.registropedido.quimico.pe.i18n.i18n_ing";
			} else {
		...
		_resetDetailState: function () {
			const oM = this.getOwnerComponent().getModel("oModelProyect");
			if (!oM) return;

			// Limpieza dura del estado de detalle (sin tocar filtros de Main)
			oM.setProperty("/oMaterial", []);
			oM.setProperty("/oMaterialUI", []);
			oM.setProperty("/oMaterialSelect", []);
			oM.setProperty("/oMaterialBase", []);
			oM.setProperty("/oCantidades", {});
			oM.setProperty("/oConditionsSAP", []);
			oM.setProperty("/oItemsOutSAP", []);
			...
			// También apaga modo edición
			oM.setProperty("/isDetailEdit", false);
			oM.setProperty("/isFormEnabled", false);

			oM.refresh(true);
		},
		_resetFormClientState: function (oM) {
			// Cliente / cabecera
			oM.setProperty("/oDatClient", {});
			oM.setProperty("/oDireccionCliente", {});
			oM.setProperty("/oCreditoCliente", {});
			oM.setProperty("/oClientData", {});
			oM.setProperty("/oAgenciasClienteFiltradas", []);
			oM.setProperty("/oDestinosCliente", []);
			oM.setProperty("/oFinalDestinosCliente", []);

			// Formulario principal (FormClient usa esto)
			oM.setProperty("/isFormEnabled", false);

			// IMPORTANTE: inputForm es el corazón del “pegado”
			oM.setProperty("/inputForm", {
				tipDocument: "",
				txtTipDocument: "",
				...
				transporteText: "",
				direccionAgencia: "",
				direccionAgenciaText: "",
				direccionAgenciaAddrText: "",

				destinoQuimicos: "",
				destinoQuimicosText: "",
				sedeFinalDiferente: false,
				destinoFinal: "",
				destinoFinalText: "",
				...
				// cualquier otro campo que uses en FormClient
			});

			// Filtros / fechas (si quieres volver a “hoy”)
			oM.setProperty("/Main/filter/fechInicio", new Date());
			oM.setProperty("/Main/filter/fechFin", null);

			// UI adicional si aplica
			oM.setProperty("/inputFormBackup", null);
		},
		_resetFormClientModel: function (oM) {
			// datos cliente
			console.log("en teoria si debe limpiar")
			oM.setProperty("/oDatClient", {});
			oM.setProperty("/oDireccionCliente", {});
			oM.setProperty("/oCreditoCliente", {});
			oM.setProperty("/oClientData", {});
			...
			// inputForm (son strings en tu vista)
			oM.setProperty("/inputForm", {
				tipDocument: "",
				isTipDocumentEnabled: true,

				purchaseOrder: "",
				ocExpDate: "",          // ✅ string porque DatePicker usa value
				...
				showSeparationDates: false,
				fechFin: ""             // ✅ string porque DatePicker usa value
			});

			// Main/filter (DatePicker usa dateValue => Date)
			oM.setProperty("/Main/filter/fechInicio", new Date()); // o null si quieres vacío
			oM.setProperty("/Main/filter/fechFin", null);
		},
		_resetFormClientControls: function () {
			// Inputs / TextAreas
			console.log("en teoria si debe limpiar")
			this.byId("ObservationsPed")?.setValue("");
			this.byId("ObservationsDelivery")?.setValue("");

			// DatePickers con value (string)
			this.byId("OCFechExpiration")?.setValue(""); // ✅ importante

			// DatePickers con dateValue (Date)
			this.byId("FechInit")?.setDateValue(new Date()); // o null si quieres vacío
			this.byId("FechEnd")?.setValue("");              // porque FechEnd usa value

			// Combos / selects
			this.byId("cbCondPago")?.setSelectedKey("");
			this.byId("DestinationTexChemicals")?.setSelectedKey("");
			this.byId("DestinationFinishChemicals")?.setSelectedKey("");
			this.byId("inputTransporte")?.setSelectedKey("");
			this.byId("cbDireccionAgencia")?.setSelectedKey("");

			// Tokens / uploader
			this.byId("fileTokenInput")?.removeAllTokens();
			const pi = this.byId("piUpload");
			if (pi) { pi.setPercentValue(0); pi.setDisplayValue("0%"); pi.setVisible(false); }
		},
		_clearManualFields: function () {
			const oModel = this.getView().getModel("oModelProyect");

			// Limpia inputs bindeados
			...
			// Opcional: por si el input quedó “pegado” visualmente
			if (this._oLastPrecioManualInput) this._oLastPrecioManualInput.setValue("");
		},
		_getCtrl: function (sId) {
			return this.byId(sId) || sap.ui.getCore().byId(this.getView().getId() + "--" + sId);
		},
		_getQtyNumberFormat: function () {
			if (!this._oQtyNumberFormat) {
				this._oQtyNumberFormat = sap.ui.core.format.NumberFormat.getFloatInstance({
					minFractionDigits: 2,
					maxFractionDigits: 2,
		...
		_parseQtyInput: function (v) {
			...
			// dejar solo número con decimal punto
			const n = parseFloat(s);

			return isNaN(n) ? 0 : n;
		},

		_formatQtyInput: function (v) {
			const n = typeof v === "number" ? v : this._parseQtyInput(v);
			return this._getQtyNumberFormat().format(n || 0);
		},
